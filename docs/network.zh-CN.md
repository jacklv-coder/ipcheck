...
- 从 Cloudflare 下载两个限量样本（256 KiB 和 2 MB）；
- 上传两个全零限量样本（128 KiB 和 1 MB）；
- 使用报告中显示的代理/网络路径；
- 将每个传输请求的时间限制在最多 8 秒；
- 对有效样本速度取平均值，并显示样本可信度；
- 为开发适配分贡献 20 分，重复出现低速结果时还会限制总分上限。

`https://speed.cloudflare.com/__down` 是
[Cloudflare 官方测速引擎](https://github.com/cloudflare/speedtest)使用的下载
API。官方引擎会执行范围更广的自适应测速。ipcheck 每个方向只使用两个小
尺寸，以控制耗时和流量；它不会复刻 Cloudflare 的完整测速流程。
...
上下行共同构成 20 分的“工程传输”维度。重复出现明显受限的结果会限制原本
较高的总分；不可用或跳过的方向保持中性，并标记为未测量。这样既让传输表现
具有实质权重，也让真实 AI 服务路径继续作为主要信号。详见
[评分与结论规则](scoring.zh-CN.md)。
